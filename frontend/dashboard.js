    ...
    fetch('http://localhost:3000/api/estatisticas')
        ...
        .then(dados => {
            ...
            dados.forEach(linha => {
                ...
                tr.innerHTML = `
                    <td style="font-weight: bold;">${linha.atleta}</td>
                    <td style="color: #FFEB3B;">${linha.gols || 0}</td>
                    <td style="color: #9C27B0;">${linha.finalizacoes || 0}</td>
                    <td style="color: #2196F3;">${linha.passes_certos || 0}</td>
                    <td style="color: #F44336;">${linha.passes_errados || 0}</td>
                    <td style="color: #FF9800;">${linha.interceptacoes || 0}</td>
                `;
                tabelaCorpo.appendChild(tr);
            });
...
function desenharGrafico(dadosDaApi) {
    const nomes = dadosDaApi.map(linha => linha.atleta);
    const passesCertos = dadosDaApi.map(linha => linha.passes_certos || 0);
